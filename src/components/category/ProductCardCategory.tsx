...
const ProductCardCategory = ({ slug, isNew, name, description, reverse }: ProductCardCategoryProps) => {
  ...
  return (
    <div className={`flex items-center ${reverse ? 'flex-row-reverse' : ''} md:flex-col`}>
      <img className="w-[46%] rounded-lg md:h-[46%] md:w-full" src={renderProductImages(slug)} alt="" />
      <div
        className={`w-[54%] text-left ${reverse ? 'pr-32' : 'pl-32'} md:h-[54%] md:w-full md:px-16 md:pt-12 md:text-center sm:px-0 sm:pt-8`}
      >
        {isNew ? <p className="overlineText !opacity-100 mb-4 sm:mb-6 sm:px-16 ">New Product</p> : ''}

...
const ProductCard = ({ idProduct, slug, isNew, description, name, cartName, price, className }: ProductHomeProps) => {
  ...
  const addProductToCart = (idProduct: number, quantity: number, slug: string, price: number, cartName: string) => {
    setQuantity(1);
    dispatch(
      addToCart({
        idProduct,
  ...
  const imageSizeSuffix = !isMobile && isTablet ? 'tablet' : isMobile ? 'mobile' : 'desktop';

  const renderProductImages = (slug: string) => {
    return `/assets/shared/product/${slug}/image-product-${imageSizeSuffix}.jpg`;
  };
  return (
    <div className={twMerge(`mb-40 flex items-center sm:mb-20 sm:flex-col`, className)}>
      <img className="w-[40%] rounded-lg sm:w-full" src={renderProductImages(slug)} alt="" />
      <div className="w-[60%] px-32 text-left md:pl-16 md:pr-0 sm:w-full sm:pl-0">
        {isNew ? <p className="overlineText !opacity-100 mb-4 sm:mt-8 sm:px-0">New Product</p> : ''}
        ...
        <div className="flex items-center gap-4">
          <div className=" flex h-[48px] w-fit bg-very-light-grey px-4">
            <button className="w-[24px] text-[13px] opacity-50" onClick={decrementQuantity}>
              -
            </button>
            ...
            <button className="w-[24px] text-[13px] opacity-50" onClick={incrementQuantity}>
              +
            </button>
          </div>
          <Button
            onClick={() => addProductToCart(idProduct, quantity, slug, price, cartName)}

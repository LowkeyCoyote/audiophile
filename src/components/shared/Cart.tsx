    ...
    <div className="w-[377px] rounded-lg p-8 text-black sm:w-[327px] sm:p-7">
      ...
      {cart.length !== 0 &&
        cart.map(({ idProduct, slug, cartName, price, quantity }) => (
          <div key={idProduct} className=" mb-6 flex items-center justify-between">
            <div className="flex items-center">
              <img className="w-[64px] rounded-lg" src={renderProductImages(slug)} alt="" />
              <div className="ml-4 flex flex-col">
                <p className="font-bold opacity-100">{cartName}</p>
                <p className="text-[14px] font-bold opacity-50">{`$ ${parseInt(price).toLocaleString('en-US')}`}</p>
              </div>
            </div>
